...
search_service_name=os.environ["Azure_SearchAI_Name"]
index_name=os.environ["Azure_SearchAI_IndexName"]
admin_api_key=os.environ["Azure_SearchAI_Key"]
...
def run_conversation(question,feedback):
    ...
    if "function_call" not in response_message:
        ...
        feedback(result1)
        return [{'role':'assistant','content':result1}]
...
'''
    ...
    return [{'role':'assistant','content':ret,'context':citText}]
